...
- **Phase 1（已完成核心链路）**：服务端 core/auth（scrypt 密码 + HMAC token）+ gateway（OpenAI/Anthropic 兼容转发、SSE 流式透传、跨协议非流式转换、usage 落库）+ usage（聚合/明细 API）+ 虚拟 Key（签发/吊销/配额限流），18 项 e2e 全过（`pnpm --filter @teamai/server test:e2e`）；客户端登录 + Agent 页聊天（模型选择 → 网关 → 流式出字）+ CLI 检测 + Kimi ACP 长连接适配器（JSON-RPC 2.0 over stdio，initialize/session/new/session/prompt/session/update/session/cancel 全流程）+ Claude headless 适配器（stream-json 输出解析 + 权限提示协议：interactive 模式 `--input-format stream-json`，control_request/can_use_tool → UI 询问或 autoApprove → control_response allow/deny，支持多轮 sendUserMessage）；Agent 工作台 CLI 模式端到端接线完成——主进程 agentRunner 统一桥接 ACP 单例（Kimi）与 Claude headless，IPC `agent:run/stop/permission` + 渲染进程事件订阅，Agent 页支持网关/本地 CLI 双模式切换、CLI 与工作目录选择、权限请求弹窗（允许/拒绝）、思考/工具调用流式展示、停止按钮，会话存档抽屉支持列表查看与一键恢复。
- **Phase 2（已完成核心）**：Git 托管落地——bare 仓库创建/删除、smart HTTP（`git http-backend` CGI 桥接 + Basic/Bearer 鉴权 + 路径防穿越）、commits/tree 浏览 API；仓库级成员权限——visibility（team/private）+ repo_members（owner/member），列表过滤、commits/tree、clone/push（smart HTTP 403）、在线环境建环境全链路拦截，可见性切换与成员管理端点 + 审计埋点，控制台仓库页含可见性标签/切换/成员管理弹窗。9+25 项 e2e 全过（`test:e2e:git` 含真实 clone/push/多分支，`test:e2e:repos` 覆盖私有仓库隔离全流程）；分支管理——`GET /api/repos/:id/branches` + commits/tree 支持 `?ref=`，客户端提交历史抽屉带分支选择器；客户端「项目仓库」页（列表/新建含可见性/克隆/复制地址/提交历史抽屉）；会话存档 API + Agent 页「保存会话」按钮。
- **Phase 3（已完成核心）**：自研轻量 IM 落地——单聊（自动去重）/群组、REST 历史 + WebSocket 实时广播（token 认证、在线连接管理）、未读数与已读标记、消息悬停「喂给 AI」跳转 Agent 工作台；AI 角色引擎：群内 @角色名 触发 → 带人设 prompt + 最近 30 条上下文调用网关 → 以角色身份回复并广播、用量计入触发者；文件/图片消息已落地（20MB 上限、文件名清洗、路径防穿越、图片内联展示）。15+11 项 e2e 全过（`test:e2e:im` 29 项含双端 WS 实时收发、三种触发模式、typing、分页，`e2e-files.mjs` 覆盖上传/下载/鉴权/越界/列表隔离/删除权限）；AI 角色触发模式已落地——@提及 / 关键词（大小写不敏感，必填校验）/ 自动回复每条消息，PATCH 端点支持停用/改名/改关键词，防循环（角色消息不再触发角色），客户端角色表单含触发方式选择与关键词标签输入、启停开关；typing 指示端到端落地（WS 广播排除发送者、非成员静默丢弃、客户端输入节流上报 + 3.5s 过期指示条、Electron IPC `im:typing`）；消息分页加载 UI（顶部「加载更早消息」按钮，`?before=` 游标）。
- **Phase 4（已完成核心）**：在线环境（进程级沙箱）落地——从 Git 仓库一键克隆出服务端工作区、启动命令进程管理（spawn + 进程组终止 + 退出状态自动回收 + 重启 reconciliate）、环形缓冲日志实时拉取、环境内一次性命令执行、成员只能操作自己的环境（管理员可见全部）；审计日志落地——audit_logs 表 + 登录成功/失败、建用户、Provider/Key/仓库/环境增删改全量埋点，`/api/admin/audit` 支持按动作前缀与成员过滤；服务端内置管理控制台（`/admin` 单页，零依赖）覆盖用量/Provider/虚拟 Key/成员/仓库/环境/审计七个面板。17 项 e2e 全过（`node scripts/e2e-envs.mjs`）；管理控制台 UX 已重写为侧边栏导航 + toast 通知 + 自定义模态框 + 加载态的现代界面（无原生 alert/confirm）；electron-builder 打包配置就绪并已在沙盒实测出包：主进程生产化（单实例锁、窗口图标、外链走系统浏览器、退出时回收 CLI 会话）、`scripts/gen-icon.mjs` 程序化生成 256px 图标（PNG + ICO），Windows 目标含 NSIS 安装包（`TeamAI-Setup-x.y.z.exe`，向导式可改目录）与便携版（`TeamAI-Portable-x.y.z.exe`），`pnpm --filter @teamai/client dist:win` 产出至 `apps/client/release/`；注意 NSIS 安装包在 Linux 上交叉构建需要可运行 32 位程序的 wine（安装向导 stub 为 32 位，需执行一次以捕获卸载器），portable/zip 无此要求；在线环境 runner 已抽象为 process/docker 双后端——`TEAMAI_ENV_RUNNER=auto|process|docker`，docker 后端实现 `docker run -d`（挂载工作区至 /workspace、镜像可用 `TEAMAI_ENV_IMAGE` 配置）+ `logs -f` 日志流 + `wait` 退出回收 + `rm -f` 停止 + `exec` 一次性命令 + 重启后容器状态 reconcile，无 docker 时自动降级进程级；Web 端完整客户端落地——服务端直接托管 `apps/client/dist`（SPA fallback + MIME 表 + 路径穿越防护 + API/管理台路径保留），浏览器打开 `/` 即是完整 TeamAI 客户端，无需 Electron；**后按用户要求移除**：服务端回归纯 API + 内置管理控制台，`/` 返回 JSON 服务信息，`/admin` 为控制台，未知路径一律 404 JSON，客户端仅保留 Electron 桌面形态；文件管理页落地——服务端新增文件列表（成员看自己/管理员 `?all=1` 看全部）与删除（owner 或管理员，审计 file.delete）API，客户端「文件管理」页支持上传/预览图片/下载/删除。累计 130 项 e2e 测试全过（新增 `test:e2e:web` 9 项、files 扩至 18 项）；客户端「连接而非依赖」架构落地——不再内置默认服务端地址（`api.ts` 与 Electron `serverClient.ts` 均无 hardcode），Electron 登录页必填服务端地址（存 localStorage，设置页可改且即时生效），并支持「暂不连接，仅使用本地 Agent CLI」离线模式（`teamai_offline` 持久化，仅暴露 Agent 工作台与设置页，可随时回登录页连接服务端）；vite `base: "./"` 修复 file:// 协议下白屏，主进程 did-fail-load 兜底错误提示；桌面端 UI 已参照 ai-cli-hub 全面重构——全局深色主题（antd darkAlgorithm + 定制色板 #141416/#1e1e20/#242426，品牌主色靛紫 #6366f1），主导航改为 64px 窄图标栏（Tooltip + 激活指示条 + 底部设置/用户头像），Agent 工作台为三段式布局：左侧会话栏（新增对话主按钮、搜索、按今天/昨天/本周/更早分组的存档会话列表、点击恢复）+ 中间聊天流（用户/AI 头像气泡、可折叠思考过程块、工具调用卡片带状态图标、流式光标）+ 底部圆角输入盒（模式/模型标签、圆形发送/停止按钮），登录页改为居中深色卡片 + 渐变 Logo；用量统计页参照 ai-cli-hub 重构——新增 `GET /api/usage/stats`（范围/今日/本周聚合 + 按日热力序列 + 按模型/成员/CLI 分布，成员强制只能看自己、管理员可全员或按成员过滤，summary/records 同步补齐隔离），客户端为时间筛选（今天/近7天/近30天/全部）+ 四张统计卡 + GitHub 风格按日热力图（17 周、悬停明细、绿阶图例）+ 按模型/成员/CLI 三个纯 SVG 环形图（占比图例），数字按 M/k 缩写。
